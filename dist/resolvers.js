"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.resolvers = void 0;
var Post_1 = require("./models/Post");
// Import the Post model. 동일한 디렉토리에 있는 파일이라고 가정한다.
var model = require("mongoose").model;
// const Post = require("./models/Post");
// resolvers는 그래프큐엘 쿼리와 뮤테이션에 응답하는 방법을 정의한다.
// schema.js 에 정의한 typeDefs에 상응하네..!
exports.resolvers = {
    // # 쿼리 리졸버 객체는 read 작업(operation)을 처리한다.
    Query: {
        // 1. 모든 데이터를 가져오기 위한 리졸버
        posts: function () { return __awaiter(void 0, void 0, void 0, function () {
            var error_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        return [4 /*yield*/, Post_1.Post.find()];
                    case 1: 
                    // 모든 포스트들을 찾고 반환하기 위해 Post 모델(몽구스)을 사용한다.
                    return [2 /*return*/, _a.sent()];
                    case 2:
                        error_1 = _a.sent();
                        throw new Error("Error fetching posts from the database");
                    case 3: return [2 /*return*/];
                }
            });
        }); },
        // 2. id로 포스트를 가져오기 위한 리졸버
        post: function (parent_1, _a) { return __awaiter(void 0, [parent_1, _a], void 0, function (parent, _b) {
            var error_2;
            var id = _b.id;
            return __generator(this, function (_c) {
                switch (_c.label) {
                    case 0:
                        _c.trys.push([0, 2, , 3]);
                        return [4 /*yield*/, Post_1.Post.findById(id)];
                    case 1: 
                    // id로 특정한 포스트를 찾고 반환하기 위해 Post 모델을 사용한다.
                    return [2 /*return*/, _c.sent()];
                    case 2:
                        error_2 = _c.sent();
                        throw new Error("Error fetching post with ID: ".concat(id, " from the database"));
                    case 3: return [2 /*return*/];
                }
            });
        }); },
    },
    // 뮤테이션 리졸버 객체는 쓰기 연산을 처리한다.
    Mutation: {
        // 1. 새로운 포스트를 생성하는 리졸버
        createPost: function (parent_1, _a) { return __awaiter(void 0, [parent_1, _a], void 0, function (parent, _b) {
            var post, error_3;
            var title = _b.title, content = _b.content;
            return __generator(this, function (_c) {
                switch (_c.label) {
                    case 0:
                        _c.trys.push([0, 2, , 3]);
                        post = new Post_1.Post({ title: title, content: content });
                        return [4 /*yield*/, post.save()];
                    case 1: 
                    // 새 포스트를 데이터베이스에 저장하고 반환한다.
                    return [2 /*return*/, _c.sent()];
                    case 2:
                        error_3 = _c.sent();
                        throw new Error("Error createing a new post in the database");
                    case 3: return [2 /*return*/];
                }
            });
        }); },
        // 2. 포스트를 업뎃하는 리졸버
        updatePost: function (parent_1, _a) { return __awaiter(void 0, [parent_1, _a], void 0, function (parent, _b) {
            var error_4;
            var id = _b.id, title = _b.title, content = _b.content;
            return __generator(this, function (_c) {
                switch (_c.label) {
                    case 0:
                        _c.trys.push([0, 2, , 3]);
                        return [4 /*yield*/, Post_1.Post.findByIdAndUpdate(id, { title: title, content: content }, { new: true })];
                    case 1: 
                    // id로 포스트를 찾고 업데이트하고 업뎃한 포스트를 반환
                    return [2 /*return*/, _c.sent()];
                    case 2:
                        error_4 = _c.sent();
                        throw new Error("Error updating post with ID: ".concat(id, " in the database"));
                    case 3: return [2 /*return*/];
                }
            });
        }); },
        // 3. id로 포스트를 삭제하는 리졸버
        deletePost: function (parent_1, _a) { return __awaiter(void 0, [parent_1, _a], void 0, function (parent, _b) {
            var error_5;
            var id = _b.id;
            return __generator(this, function (_c) {
                switch (_c.label) {
                    case 0:
                        _c.trys.push([0, 2, , 3]);
                        return [4 /*yield*/, Post_1.Post.findByIdAndDelete(id)];
                    case 1: 
                    // id로 포스트를 찾고 삭제하고 삭제한 포스트를 반환
                    return [2 /*return*/, _c.sent()];
                    case 2:
                        error_5 = _c.sent();
                        throw new Error("Error deleting post with ID: &{id} from the database");
                    case 3: return [2 /*return*/];
                }
            });
        }); },
    },
};
// 아폴로서버에서 사용하기 위한 리졸버 내보내기
// module.exports = resolvers;
